...
├── hardware/
│   ├── design/                          ← Schaltplan als Python-Modell: 24 Prüfungen + Simulationen
...
- [x] **Schaltplan V1 + Netzliste** (`hardware/schaltplan_v1.md`, `..._netzliste.csv`, **32 Netze / 52 bestückte Positionen**)
- [x] **Design als Python-Modell + Prüfungen** (`hardware/design/`): **24** Design-Regelprüfungen gegen die
...
- [x] **Lichtsensor extern + Licht-Gate** (13.09.2026): Bewässerung nur in der Dunkelphase.
      Stecker **J7** (JST-XH 3P wie J2) mit **R_LIGHT 10 kΩ** (offener Stecker ⇒ 0 V ⇒ „dunkel",
      Bewässerung bleibt erlaubt) · **R_LIGHT_S 1 kΩ** + **C_LIGHT 100 nF** am ADC · Sensor
      an **SENSOR_PWR** (geschaltet, keine Standby-Verschlechterung) · ADC auf **IO4 (ADC1_CH4)**.
       Firmware-Fail-safe inkl. Zeitfenster-Fallback und Telegram-Alarm. **Pin-Korrektur:** IO4/IO5
       sind keine boot-kritischen Strapping-Pins (nur GPIO8/9/15). Neue Prüfungen + Mutationstest.
- [x] **Pinordnung GND–VCC–SIG** (13.09.2026): **J2/J7** auf
      1 = GND · **2 = VCC** · 3 = Signal umgestellt (nur der mittlere Pin ist gegen Umdrehen
      invariant — ein verkehrt gecrimpter Stecker kann so **nie** 3,3 V auf einen MCU-Pin legen).
- [x] **Rückbau der GPIO-Erweiterung** (14.09.2026): die am 13.09. ergänzte Erweiterung
      (**J8** I²C mit **Q2** Load-Switch, **J9** Reserve-Analog, **J10** Reserve-Digital,
      **TP7–TP11**) wurde auf Wunsch des Nutzers wieder entfernt — aus Netzliste, Designmodell,
      Generatoren, EasyEDA-Schaltplan und PCB. Geblieben sind Lichtsensor (J7), Pinordnung
      GND–VCC–SIG und die einzeln verdrahteten U1-Massepads → **32 Netze / 52 bestückte
      Positionen**; Schaltplan live neu gebaut (58 Bauteile, `sch gate` **pass**), PCB per
      `pcb import-changes` nachgezogen.
